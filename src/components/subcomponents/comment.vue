<template>
    <div class="comment-container">
         <h2>发表评论</h2>
         <hr>
         <textarea name="" id="" maxlength="120" placeholder="请输入要bb的内容(最多120字)"></textarea>
         <mt-button size="large" type="primary">发表评论</mt-button>
         <ul>
             <li v-for="(item, index) in comments" :key="item.add_time">
                 <div class="comment-info">第{{ index+1 }}楼&nbsp;&nbsp;用户：{{ item.user_name }}&nbsp;&nbsp;发表时间：{{ item.add_time | dateFormat }}</div>
                 <div class="comment-text"><p>{{item.content === '' ? '此用户很懒，嘛都没说': item.content}}</p></div>
             </li>
